...
func (s *SimpleBankDB) TransferTx(ctx context.Context, arg TransferTxParams) (TransferTxResult, error) {
	...
	err := s.execWithContext(ctx, func(q *simplebanksql.Queries) error {
		var err error
		// Create transfer.
		result.Transfer, err = s.queries.CreateTransfer(ctx, simplebanksql.CreateTransferParams{
...
func (s SimpleBankDB) updateBalance(ctx context.Context, fromAccountID, fromAmount, toAccountID, toAmount int64) (fromAccount, toAccount simplebanksql.Account, err error) {
	...
	fromAccount, err = s.queries.AddAccountBalance(ctx, simplebanksql.AddAccountBalanceParams{
		ID:     fromAccountID,
		Amount: fromAmount,
	})
	...
	toAccount, err = s.queries.AddAccountBalance(ctx, simplebanksql.AddAccountBalanceParams{
		ID:     toAccountID,
		Amount: toAmount,
	})
